...
#    def read(self,config_string):
    ...
    def dissociation(self,config=False, step=False, atomA=False,atomB=False):

        if config is False: #Meaning need the default one
        ...
        while dis_min <= dis_max:
            self.resize(n_configs=1, configs=config, )
            configs_count = configs_count + 1
