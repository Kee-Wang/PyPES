...
class configs_hwww_qct():
    ...
    def __init__(self, train_x, dip=False, first_n_configs=None, xlim=[-10E10, 10E10],col=3):
        ...
        molecule_coord = list()
        config_mark = list()
        traj_info=list() #To record steps
        f = open(train_x)
        for line in f:
            ...
            if block_line >= 3 and block_line <= block_size:
                if line.split()[0] != elements[block_line - 3]:
                    print("Type error: Element in line " + str(line_count) + " is : " + str(
                        line) + '. Skipping.')  # Check
                    config_mark.append(len(configs))
                try:
        ...
        print('Broken config: {:d}'.format(self.broke))
    ...
    def write(self, filename, configs=False):  # Default value has to be after non-default value.
        ...
        for config in configs:  # Write config by config
            ...
            if self.dip:  # Print energy and dipole
                f.write('{:14.8f}{:14.8f}{:14.8f}{:14.8f}'.format(config[1][0][0], config[1][1][0], config[1][1][1],
                                                                  config[1][1][2]) + '\n')
    ...
    def threshold_energy(self, configs, lower=False, upper=False):
        ...
        if energy_lowest > energy_highest:
            print(
                'Boundary error: lower energy bound is greater then upper energy bound. Returning origianl configurations.')
            self.error_message()
            return configs
        else:
            for config in configs:
            ...
            print('There are ' + str(configs_new_count) + '/' + str(self.configs_count) + '(' + str(
                percentage) + '%) configs between ' + str(energy_lowest) + ' and ' + str(energy_highest) + ' Hartree')
    ...
    def resize(self, n_configs=False, configs=False, first_n_configs=False, monomers=False, dis_lower=False,
               dis_upper=False, dis_new_lower=False, dis_new_upper=False):
        import numpy as np
        configs_new = list()
        configs = self.configs_check(configs)
        ...
        while configs_ok_count < n_configs:
            ...
            if dis_lower <= dis <= dis_upper:
                ...
                print(
                    'New distance ({:5d}): {:5.2f}'.format(configs_ok_count, self.distance(config_new, atom_A, atom_B)))
    ...
    def plot2(self, clip_rate=False, binwidth=False, configs=False):
        ...
        ax.annotate('\nTail {:3.2f}%\n< {:7.1f}cm$^{{-1}}$'.format(clip_out * 100, E_min), xy=(E_min, 0),
                    xytext=(E_min - (E_max - E_min) * 0.05, count_highest * 0.2), arrowprops=dict(arrowstyle="->"))
        ax.annotate('\nHead {:3.2f}%\n> {:7.1f}cm$^{{-1}}$'.format(clip_out * 100, E_max), xy=(E_max, 0),
                    xytext=(E_max + (E_max - E_min) * 0.05, count_highest * 0.2), arrowprops=dict(arrowstyle="->"))
        # ax.xlabels[-1] = '300+'
    ...
    def submit(self, configs=False, file='dimer', v2b=False, basis='avtz', ab='ccsd(t)-f12', usr='kee', monomerA=None,
               monomerB=None):
    ...
    def compared(self, configss, atomA=3, atomB=6, s=100, title='', color='b', marker='.', label='default', xmin=None,
                 xmax=None, ymin=None, ymax=None, xmin2=None, xmax2=None, ymin2=None, ymax2=None, loc=None,
                 fontsize=12):
        ...
        for n in range(0, n):
            ...
            ax2.scatter(dis[n], ecompare[n], s=s, color=color[n], marker=marker[n], label=label[n], facecolors='none',
                        linewidth='2')
