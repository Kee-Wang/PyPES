...
real::miu,Et,Etsum(4), Etj4, Etj6, speedj4,speedj6
!real::speed(4)
Jcount = 0
Jsum = 0
Jrot = 0
...
kin_hwww = calc_kine(mass, com_velc) * aucm
...
iflag(3) = 1
!write(*,*) kine_h*aucm*sum(mass(:))/(sum(mass(:))-sum(mass(10:11))),kine_h*aucm+ kine_www*aucm
!write(*,*) kin_hwww,kine_h*aucm+kine_www*aucm, kine_h*aucm! kine_www*aucm
!write(*,*) E_given, Erot_hcl, Erot_www, pot_h, pot_www
!write(*,*) E_given, (Erot_hcl+ Erot_www)*aucm+ pot_h+ pot_www
!write(*,*) E_given - (Erot_hcl + Erot_www)*aucm - pot_h - pot_www
end if
...
!write(*,*) Et
!write(*,*) E_given - pot_h - pot_www - (Erot_www + Erot_hcl) - kin_hwww
!write(*,*) 0.5*miu*sum((vcom_h-vcom_www)**2)*aucm
Etsum(i) = Etsum(i) + 0.5*miu*sum((vcom_h-vcom_www)**2)*aucm
!Record
