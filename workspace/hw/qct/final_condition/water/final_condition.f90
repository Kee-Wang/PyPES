...
!=================================
!Take x and v, returns speed, Ekine and Erot
!================================


subroutine fin_cond(mass,x,v,speed,Ekine,Erot,js,abc,evect)
...
com_velc=com_velc/sum(mass) !Center of mass velocity
...
!For Erot
    ...
    call rotn(0,natm,xx,vv,mass,js,erot,omega,abc, evect)
end subroutine
...
real function j331(abc)
real::abc(3),a,b,c
A = abc(1)
B = abc(2)
C = abc(3)
j331 = 5*A + 2*B + 5*C + 2*sqrt(4*(A-C)**2 - (A-B)*(B-C))
end function

real function j321(abc)
real::abc(3),a,b,c
A = abc(1)
B = abc(2)
C = abc(3)
j321 = 2*A + 5*B + 5*C + 2*sqrt(4*(B-C)**2 + (A-B)*(A-C))
end function



real function j322(abc)
real::abc(3),a,b,c
A = abc(1)
B = abc(2)
C = abc(3)
