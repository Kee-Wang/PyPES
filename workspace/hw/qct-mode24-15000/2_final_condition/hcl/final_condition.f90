...
subroutine fin_cond(mass,x,v,speed, Ekine, Erot,js,com_velc)
...
!This speed is the speed of reference, speed of 
!COM. Ths is a single 3D vector

!Before vcom, the v contins the COM translation
!and rotation and vibraiton kineteic
    call xcom(mass,x)
    call vcom(mass,v)
!After vcom, translation speed of COM is removed,
!What's left is for internal kinetic energy only
!

Ekine = calc_kine(mass(:),v) !The total kin after com
    do i=1,natm
       xx(i,:)=x(3*i-2:3*i)
       vv(i,:)=v(3*i-2:3*i)
    end do
