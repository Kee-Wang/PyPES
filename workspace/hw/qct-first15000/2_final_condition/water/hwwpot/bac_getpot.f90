...
!    do i=1,3
!       v_w(3*i-2:3*i)=v_w(3*i-2:3*i)-com_velc
 !   end do
!calculate vibrational energy, so that to determin if zpe violated
!nwat = 1
!nhcl = 0
!Evib = calc_kine(mass_w,v_w)*aucm + (f(xxw)*aucm - E_w_ref )  !Eint
...
!    do i=1,8
!       v_hww(3*i-2:3*i)=v_hww(3*i-2:3*i)-com_velc
!    end do

!nwat = 2
!nhcl = 1
!Evib_hww = calc_kine(mass_hww,v_hww)*aucm +  (f(xxhww)*aucm - E_hww_ref ) !Eint
...
Jmoi = abs(nint(matmul(transpose(evect),j_w)))
Jall = nint(sqrt(sum(real(Jmoi)**2)))!Sum of Jmoi
jmoi_real = matmul(transpose(evect),j_w)
Jall_real = sqrt(sum(real(jmoi_real)**2))
...
write(*,*) mass_w, v_w, calc_kine(mass_w,v_w)
nwat = 2
nhcl = 1
...
-(Ekine + Ekine_hww)
write(*,*) 'Eint2', Erot + Erot_hww + Evib + Evib_hww
