...
save = './thesis/FIG_S1a_j4speed.eps'
ymax2 = 1.2
bin = [50,50,50,50]
note = ['(a)',171,'311 m/s']
#hcl_j_speed_plot(file_expj4,file_hclj4,save,ymax2,bin,index=2)

save = './thesis/FIG_S1b_j6speed.eps'
ymax2 = 1.2
bin = [40,40,40,40]
label = ['No Constraint','HCl only ZPE','Soft ZPE','Hard ZPE']
note = ['(b)',96,'260 m/s']
...
def hcl_j_distribution(file2,save):
   ...
   # ax.legend()
    ...
    #ax2 = ax.twinx()
    #ax.errorbar(np.nan,np.nan,yerr=np.nan, fmt='ko',capsize=3,elinewidth=0.7, markeredgewidth=0.5, label='Exp',markersize=3)
    ax.set_ylim([0, ymax2])
    ax.set_ylabel(ytitle2)
    multi=[1,5,10,100]
    unique = [0,0,0,0]
...
hcl_j_distribution(file_hcl,save = './thesis/FIG4_hcl_j_distribution.eps')
